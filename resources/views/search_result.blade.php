...
<div class="searchFormWrapper" style='background-image: url("{{ asset('assets/images/ui/search-bg.jpg') }}")'>
    <div class="container">
        <section class="searchForm">
            <h2>Search for talent all around you</h2>
            <!-- sectionHeader -->
            ...
            <div class="label">
                ...
                <input name='sort_by' type="hidden" id="sort-option" value="{{@Request::input('sort_by')}}" />
...
<!-- searchWrapper -->
<div id="searchResultDivId" class="searchResultsWrapper">
    <div class="container">
        @include('partials.refineSearch')
        <section class="searchResults">
            ...
            <div class="sortForm">
                    <label>
                        <span>Sort By:</span>
                        <select id="search-sort-options" name="sort_by">
                            <option value="distance asc" <?php if(@Request::input('sort_by') == 'distance asc') echo "selected" ?> >Distance</option>
                            <option value="rating desc" <?php if(@Request::input('sort_by') == 'rating desc')  echo "selected" ?> >Average Rating</option>
                            <option value="updated_at desc" <?php if(@Request::input('sort_by') == 'updated_at desc')  echo "selected" ?> >Latest update</option>
                            <option value="price_average desc" <?php if(@Request::input('sort_by') == 'price_average desc')  echo "selected" ?> >Price Average </option>
                        </select>
                    </label>
            </div>
            <!-- sortForm -->
            <ul class="users usersProfiles">
                @if(count($profiles))
...
We're constantly expanding our platform, so we'll be able to better assist your request shortly.
...
Thank you for your patience! 
            ...
            @if(count($profiles) == 9)
                <div class="loadMore">
                    <a href="#" class="buttonDefault loadMoreProfilesAdvancedSearch"><i class="icon">&#xf0e2;</i><span>Load More</span></a>
                </div>
                <!-- loadMore -->
            @endif
        </section>
